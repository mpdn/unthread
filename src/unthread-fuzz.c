...
int main(int argc, char *argv[]) {
    ...
    char *endptr;
    while(parsing &&
        (opt = getopt_long(argc, argv, ":p::s:f::h", long_options, &option_index)) != -1
    ) {
        switch (opt) {
            case 'p':
    ...
    if (seeds_path != NULL) {
        ...
        CHECK(fseek(seeds, 0, SEEK_SET) == 0, "Failed seeking to beginning of seeds file: %s",
            strerror(errno));
    }
    ...
    CHECK(
        posix_spawn_file_actions_init(&action) == 0 &&
        posix_spawn_file_actions_addopen(&action, STDOUT_FILENO, "/dev/null", O_WRONLY, 0) == 0,
        ...
        while (running < parallelism) {
            ...
            if (seeds != NULL && fscanf(seeds, "%32s\n", process.seed_str) > 0) {
                ...
                if (fuzz && (fuzz_iterations == -1 || fuzz_iterations > 0)) {
                    ...
                    char seed[16];
                    CHECK(fread(&seed, sizeof(seed), 1, noise), "Failed reading random seed: %s",
                        strerror(errno));
            ...
            CHECK(posix_spawnp(&process.pid, child_argv[0], &action, NULL, child_argv,
                (char*[]){seed_str, NULL}) == 0, "Failed spawning process: %s", strerror(errno));
            ...
            if (processes[i].pid == pid) {
                ...
                if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) {
                    ...
                    if (seeds != NULL && process.generated) {
                        ...
                        CHECK(fclose(seeds), "Failed closing seeds file: %s", strerror(errno));
                    }
